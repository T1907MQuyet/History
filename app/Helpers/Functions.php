...
/**
 * change tiêu đề trang về dạng không dấu
 * @param $title
...
function imageThumb($path, $type = 0)
{
    if($type)
...
{
    $image = new \App\Helpers\simpleImage($path);
    $image->overlay(public_path("admin/dist/images/watermark.png"), 'bottom right', .8, 0, 0)->save();
}

function resizeImage($path, $newpath, $x, $y, $type = 0)
{
    $image = new \App\Helpers\simpleImage($path);
    $image->load($path);
    if(!$type)
...
/**
 * hiển thị dữ liệu breadcrumb
 * @return string
 */
function showBreadcrumb($data = null)
{
    $result = '<ol class="breadcrumb">';
    $result = $result . '<li itemscope="" >
